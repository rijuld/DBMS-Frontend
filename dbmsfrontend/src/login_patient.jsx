...
function Login_patient() {
  ...
  const login = () => {
    axios.post("http://localhost:5000/login_patient",{
      pid:pid,
      password:password,
    }).then((response) => {
      ...
      setloginStatus(response.data[0].pid);
    ...
    <div className="col-md-9 col-lg-12 col-xl-10" style={{background: 'black', borderRadius: 16}}>
      <div className="card shadow-lg o-hidden border-0 my-5">
              ...
              <div className="p-5" style={{borderRadius: 26}}>
                ...
                <form className="user">
                  <div className="form-group">
                    <input className="form-control form-control-user" type="number" id="exampleInputEmail" placeholder="Enter ID..." name="ID" onChange={(e) =>{
        ...
        setlpass(e.target.value); 
                  ...
                  <button onClick = {login} className="btn btn-primary btn-block text-white btn-user" type="submit">
                    Login
                  </button>
                  <h1>{loginStatus}</h1>
                </form>
